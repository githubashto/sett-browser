...
public class ZircoMain extends Activity implements IWebEventListener, IToolbarsContainer, OnTouchListener, IDownloadEventsListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        /**
         * Update SETT rendering in time intervals
         */
    	
		long renderingRateUrl = 1000;
        Timer urlLoadingTimer = new Timer();
        urlLoadingTimer.schedule(new TimerTask() {
            public void run() {
            	InputMethodManager imm = (InputMethodManager) getSystemService(Context.INPUT_METHOD_SERVICE);
            	if (mCurrentWebView.isLoading() && !imm.isAcceptingText() && Controller.getInstance().getPreferences().getBoolean(Constants.PREFERENCES_SETT_RENDERING_ENABLE, true))
            		mCurrentWebView.loadSETTRender();
            }
        }, renderingRateUrl, renderingRateUrl);
        
        long renderingRefreshRate = Integer.parseInt(Controller.getInstance().getPreferences().getString(Constants.PREFERENCES_SETT_RERESH_RATE, "3000"));
        
        Timer regularTimer = new Timer();
        regularTimer.schedule(new TimerTask() {
            public void run() {
            	InputMethodManager imm = (InputMethodManager) getSystemService(Context.INPUT_METHOD_SERVICE);
            	if (!imm.isAcceptingText() && !mCurrentWebView.isLoading() && Controller.getInstance().getPreferences().getBoolean(Constants.PREFERENCES_SETT_RENDERING_ENABLE, true))
            		mCurrentWebView.loadSETTRender();
            }
        }, renderingRefreshRate, renderingRefreshRate);
