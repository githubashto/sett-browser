...
public class ZircoWebView extends WebView {
	...
	/**
	 * Inject the SETTRender javascript.
	 */
	public void loadSETTRender() {
		String settScript = Controller.getInstance().getPreferences().getString(Constants.PREFERENCES_SETT_RENDERING_SCRIPT, "native");
		if (settScript.equals("native")){
			super.loadUrl(ApplicationUtils.getSETTRenderNativeString(mContext));
		} else {
			super.loadUrl(ApplicationUtils.getSETTRenderLatinString(mContext));
		}
	}
